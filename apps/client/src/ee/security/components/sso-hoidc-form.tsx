...
const ssoSchema = z.object({
  ...
  oidcClientId: z.string().min(1, "Platform ID is required"),
  isEnabled: z.boolean(),
...
export function SsoHOIDCForm({ provider, onClose }: SsoFormProps) {
    ...
    <Box maw={600} mx="auto">
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack>
          ...
          <TextInput
            label="SSO API 地址"
            description="SSO 服务的 API 地址"
            placeholder="https://webapi-sso.example.com"
            {...form.getInputProps("oidcIssuer")}
          />
          <TextInput
            label="Platform ID"
            description="SSO 平台 ID"
